...
func (s *service) SaveCampaignImage(input CreateCampaignImageInput, fileLocation string) (CampaignImage, error) {
	...
	if input.IsPrimary {
		...
		_, err := s.repository.MarkAllImagesAsPrimary(input.CampaignID)
		if err != nil {
			return CampaignImage{}, err
		}
	}

	campaginImage := CampaignImage{}
	campaginImage.CampaignID = input.CampaignID
	campaginImage.IsPrimary = isPrimary
	campaginImage.FileName = fileLocation

	newCampaignImage, err := s.repository.CreateImage(campaginImage)
	if err != nil {
